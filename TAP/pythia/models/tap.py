...
from packages.pytorch_transformers.modeling_bert import (
    BertLayerNorm, BertEmbeddings, BertEncoder, BertConfig,
    BertPreTrainedModel
)
# LFPR
from packages.pytorch_transformers.modeling_bert_lfpr_score_calculation import BertEncoderLFPRScoreCalc
from packages.pytorch_transformers.modeling_bert_lfpr_configurable import BertEncoderLFPRConfig
...
"""
For LFPR score calculation 
"""
@registry.register_model("m4c_lfpr_calc_score")
class M4CLFPRScore(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.pretrain = self.config.pretrain
        self.mmt_config = BertConfig(**self.config.mmt)
        self._datasets = registry.get("config").datasets.split(",")
        self.selected_time_step = config.selected_time_step
        print(f"selected_time_step : {self.selected_time_step}")
    ...
    def _build_mmt(self):
        self.mmt = MMTLFPRScoreCalc(self.mmt_config)
    ...
    def forward(self, sample_list, head_mask=None,ffn_mask=None,text_mask=None):
        torch.manual_seed(0)
        fwd_results = {}
        self._forward_cmbtxt_encoding(sample_list, fwd_results)
        self._forward_obj_encoding(sample_list, fwd_results)
        self._forward_ocr_encoding(sample_list, fwd_results,text_mask=text_mask)

        self._forward_mmt_and_output(sample_list, fwd_results, head_mask, ffn_mask)
    ...
    def _forward_ocr_encoding(self, sample_list, fwd_results, text_mask=None):
        # OCR FastText feature (300-dim)
        ocr_fasttext = sample_list.context_feature_0
        if text_mask is not None:
            ocr_fasttext = ocr_fasttext * text_mask
        ocr_fasttext = F.normalize(ocr_fasttext, dim=-1)
        assert ocr_fasttext.size(-1) == 300
    ...
    def _forward_mmt(self, sample_list, fwd_results,head_mask=None,ffn_mask=None):
        # first forward the text BERT layers
        text_bert_out = self.text_bert(
            txt_inds=fwd_results['txt_inds'],
            txt_mask=fwd_results['txt_mask'],
    ...
    def _forward_mmt_and_output(self, sample_list, fwd_results,head_mask=None,ffn_mask=None):
        """
        ...
        if self.selected_time_step is None:
            ...
            dec_step_num = self.selected_time_step + 1 
        # fill prev_inds with BOS_IDX at index 0, and zeros elsewhere
        fwd_results['prev_inds'] = torch.zeros_like(
            sample_list.train_prev_inds
        )
        ...
        for t in range(dec_step_num):
            ...
            self._forward_mmt(sample_list, fwd_results,head_mask=head_mask,ffn_mask=ffn_mask)
            self._forward_output(sample_list, fwd_results)
...
class MMTLFPRScoreCalc(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.prev_pred_embeddings = PrevPredEmbeddings(config)
        self.encoder = BertEncoderLFPRScoreCalc(config)
        self.init_weights()
...
"""
LFPR configurable model
"""
@registry.register_model("m4c_lfpr_config")
class M4CLFPRConfig(BaseModel):
    def __init__(self, config):
        super().__init__(config)
        self.pretrain = self.config.pretrain
        self.mmt_config = BertConfig(**self.config.mmt)
        self.extra_mmt_config = config.mmt
        self._datasets = registry.get("config").datasets.split(",")
    ...
    def _build_txt_encoding(self):
        ...
        self.text_bert_config = BertConfig(**self.config.text_bert)
        try:
            self.text_bert_config.vocab_size = self.config.text_bert.vocab_size
        except:
            pass
        if self.config.text_bert_init_from_bert_base:
            self.text_bert = TextBert.from_pretrained(
    ...
    def _build_mmt(self):
        self.mmt = MMTLFPRConfig(self.mmt_config,self.extra_mmt_config)
    ...
    def get_optimizer_parameters(self, config):
        ...
        base_lr = config.optimizer_attributes.params.lr
        mask_lr = config.training_parameters.mask_lr

        # collect all the parameters that need different/scaled lr
        finetune_params_set = set()
        for m in self.finetune_modules[:-1]: 
            optimizer_param_groups.append({
                "params": list(m['module'].parameters()),
                "lr": base_lr * m['lr_scale']
            })
        ...
        for k,v in self.finetune_modules[-1]['module'].named_parameters():

            if 'mask' in k:
                mmt_mask_params.append(v)
            else:
                mmt_other_params.append(v)

        optimizer_param_groups.append({
            "params":mmt_other_params,
            "lr": base_lr 
        })
        finetune_params_set.update(mmt_other_params)
        optimizer_param_groups.append({
            "params":mmt_mask_params,
            "lr": mask_lr
        })
        finetune_params_set.update(mmt_mask_params)
        # remaining_params are those parameters w/ default lr
        remaining_params = [
            p for p in self.parameters() if p not in finetune_params_set
        ]
...
class MMTLFPRConfig(BertPreTrainedModel):
    def __init__(self, config,extra_mmt_config):
        super().__init__(config,extra_mmt_config)

        self.prev_pred_embeddings = PrevPredEmbeddings(config)
        # extra_mmt_config to pass cfg_head & cfg_ffn
        self.encoder = BertEncoderLFPRConfig(config,extra_mmt_config)
        self.init_weights()

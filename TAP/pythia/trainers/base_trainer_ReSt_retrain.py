...
@registry.register_trainer('base_trainer_ReSt_retrain')
class BaseTrainerReStRetraining:
    ...
    def load(self):
        ...
        ckpt = torch.load(self.config.training_parameters.resume_file)['model']
        ckptlist = list(ckpt.items())
        dict1 = {}
        for x in range(len(ckptlist)):
            dict1[ckptlist[x][0]] = ckptlist[x][1]
        state_dict = self.model.state_dict()
        flag = 0
        for k,v in state_dict.items():
            if 'text_bert.embeddings.word_embeddings.weight' in k:
                if 'module' in k:
                    flag = 1
        if flag == 1:
            state_dict['module.text_bert.embeddings.word_embeddings.weight'] =  dict1['text_bert.embeddings.word_embeddings.weight']
        else:
            state_dict['text_bert.embeddings.word_embeddings.weight'] =  dict1['text_bert.embeddings.word_embeddings.weight']
    ...
    def load_model(self):
        ...
        # Enable gradient calculation for Mask Retraining
        for k,v in self.model.named_parameters():
            if 'mask' not in k:
        ...
        # Enable gradient calculation for Partial Model Retraining 
        if self.config.training_parameters.layers != 0:
            num_layer = self.config.training_parameters.layers
            layers = np.arange(self.config.model_attributes.m4c_lfpr_config.mmt.num_hidden_layers)
            print('Layers available :',layers)
            select = layers[-num_layer:]
            print('Selected layers for Partial Model Retraining: ',select)
            lyr_str = []
            for x in select:
                lyr_str.append('mmt.encoder.layer.'+str(x)+'.')
            ...
            for x in lyr_str:
                for k,v in self.model.named_parameters():
                    if x in k:
                        ...
                        v.requires_grad=True
        for k,v in self.model.named_parameters():
            if 'ocr_ptr_net' in k or 'classifier' in k:
                v.requires_grad=True
    ...
    def train(self):
        ...
        summ_mmt = 0
        for k,v in self.model.named_parameters():
            if 'mmt.prev_pred_embeddings' in k:
                summ_mmt += v.numel()

        self.writer.write("M4C embeddings parameters :"+str(summ_mmt))
        ...
        self.writer.write("M4C encoder parameters :"+str(summ_mmt_en))
        

        summ_bert = 0
        for k,v in self.model.named_parameters():
            if 'text_bert.embeddings' in k:
                summ_bert += v.numel()
        self.writer.write('TextBERT embeddings parameters :' + str(summ_bert))

        summ_bert_en = 0
        for k,v in self.model.named_parameters():

            if 'text_bert.encoder' in k:
                summ_bert_en += v.numel()
        self.writer.write("TextBERT Encoder parameters :"+str(summ_bert_en))
        ...
        self.writer.write("Starting retraining...")

        self.writer.write("Start time : "+ str(time.time()))


        def retrain():
            while self.current_iteration < self.max_iterations and not self.should_break:
                ...
                for batch in self.train_loader:
                    ...
                    self._backward(loss)
                    self.should_break = self._logistics(report)

                    if self.should_break:
                        break

        retrain()
        self.writer.write("End time : "+ str(time.time()))

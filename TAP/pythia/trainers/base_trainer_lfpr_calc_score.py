...
@registry.register_trainer('base_trainer_lfpr_calc_score')
class BaseTrainerLFPRScoreCalc:
    def __init__(self, config):
        self.config = config

        if self.config.training_parameters.score_dir is not None:
            self.score_dir = self.config.training_parameters.score_dir
        model_name = [k for k,v in self.config.model_attributes.items()][0]
        
        if self.config.model_attributes[model_name]['selected_time_step'] is not None:
            self.selected_time_step = self.config.model_attributes[model_name]['selected_time_step']
    
        self.profiler = Timer()

        n_layers = self.config.training_parameters.nlayers
        n_heads = 12
        dim_ffn = 3072
        self.head_mask = torch.ones(n_layers,n_heads).to('cuda')
        print(self.head_mask)
        self.head_mask.requires_grad_(requires_grad=True)
        self.ffn_mask = torch.ones(n_layers,dim_ffn).to('cuda')
        self.ffn_mask.requires_grad_(requires_grad=True)
        self.head_grads = []
        self.ffn_grads = []
        self.ts = [] # list for max time steps
    ...
    def train(self):
        ...
        def calc_lfpr_grads():
            while self.current_epoch < 1:
                ...
                for idx, batch in enumerate(tqdm.tqdm(self.train_loader)):
                    ...
                    report,output = self._forward_pass(batch)

                    max_out = torch.max(output['scores'][:,self.selected_time_step,:],dim=-1)
                    max_out_sum = max_out.values.sum()
                    self._backward(max_out_sum)
        ...
        def determine_tmax_q2():
            ...
            while self.current_epoch < 1:
                ...
                for idx, batch in enumerate(tqdm.tqdm(self.train_loader)):
                    ...
                    max_val = torch.max(torch.max(output['scores'],dim=-1).values,dim=-1).values
                    ts = []
                    for val in max_val:
                        idx = (output['scores'] == val).nonzero(as_tuple=True)
                        if len(idx[1]) == 1: # single occurence 
                            self.ts.append(idx[1].item())
                        else: # more than once occurence, take the first index
                            self.ts.append(idx[1][0].item())
                    if self.should_break:
                        break  

        if self.config.training_parameters.determine_tmax_q2 == False:

            self.writer.write("Starting LFPR score calculation...")
            calc_lfpr_grads()


            self.head_grads = torch.stack(self.head_grads,dim=0)
            self.ffn_grads = torch.stack(self.ffn_grads,dim=0)        

            np.save(os.path.join(self.score_dir, "head_grads.npy"), self.head_grads.detach().cpu().numpy())
            np.save(os.path.join(self.score_dir, "ffn_grads.npy"), self.ffn_grads.detach().cpu().numpy())
            print('Saving grads ...')
        else:
            self.writer.write("Starting determining tmax_q2...")
            determine_tmax_q2()

            self.ts = np.array(self.ts)
            self.tmax_q2 = np.percentile(self.ts,50)
            self.writer.write(f"Median-max time step : {self.tmax_q2}")
            np.save(os.path.join(self.score_dir, "maximum_time_steps.npy"), self.ts)
            with open(self.score_dir + '/tmax_q2.txt','w') as f:
                f.write(str(int(self.tmax_q2)))
            #sys.exit(self.tmax_q2)
    ...
    def _forward_pass(self, batch):
        ...
        model_output = self.model(prepared_batch,head_mask=self.head_mask, ffn_mask=self.ffn_mask)
        report = Report(prepared_batch, model_output)
        self.profile("Forward time")

        return report, model_output

    def _backward(self, loss):
        self.optimizer.zero_grad()
        loss.backward()
        self.head_grads.append(self.head_mask.grad.detach())
        self.ffn_grads.append(self.ffn_mask.grad.detach())
        self.head_mask.grad = None
        self.ffn_mask.grad = None
